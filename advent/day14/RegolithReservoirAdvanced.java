...
public class RegolithReservoirAdvanced {
    ...
    private static class CaveMap {
        ...
        public Coord2D addSand(Coord2D origin) {
            if (!spaceIsFree(origin)) {
                return null;
            }

            Coord2D sandPos = origin;

            while (sandPos.y < floor - 1) {
        ...
        public String toString() {
            ...
            for (int y=lowBound.y; y<floor; y++) {
                for (int x=lowBound.x; x<=highBound.x; x++) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(caveMap.toString());
    }
}